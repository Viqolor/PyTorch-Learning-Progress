...
def train_epoch(model, dataloader, criterion, device, optimizer, epoch):
    ...
    for batch, (images, costs, labels) in enumerate(dataloader):
        ...
        if batch % 100 == 99: 
            print(f"Batch {batch + 1} | batch loss = {batch_loss / 100:.8f}, batch accuracy = {100 * pred / batch_total:.2f}%, cumulative accuracy = {100 * correct / cumul_total:.2f}%")
            batch_loss, pred, batch_total = 0.0, 0, 0

    correct = 100 * correct / cumul_total
    cumul_loss /= len(dataloader)
    print(f"Training complete: train loss = {cumul_loss:.8f}, train accuracy = {correct:.2f}%")
...
def validate_epoch(model, dataloader, criterion, device):
    ...
    loss /= len(dataloader)
    print(f"Validation complete: val loss = {loss:.8f}, val accuracy = {correct:.2f}%")
